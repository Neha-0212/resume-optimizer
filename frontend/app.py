        ...
        with tab1:
            ...
            if st.button("Login"):
                if email and password:
                    try:
                        res = requests.post(
                            "https://resume-optimizer-api-12ok.onrender.com/auth/login",
        ...
        with tab2:
            ...
            if st.button("Create Account"):
                if new_name and new_email and new_pass:
                    try:
                        res = requests.post(
                            "https://resume-optimizer-api-12ok.onrender.com/auth/signup",
                            json={
                                "email": new_email,
                                "full_name": new_name,
                                "password": new_pass
            ...
            if not uploaded_file:
                ...
                with st.spinner("Analyzing your resume..."):
                    try:
                        ...
                        res = requests.post(
                            "https://resume-optimizer-api-12ok.onrender.com/resume/analyze",
                            files=files,
                            data=data
                        )
                        if res.status_code == 200:
    ...
    if st.button("Submit Feedback", type="primary"):
        if not feedback_text or len(feedback_text) < 10:
            ...
            try:
                ...
                res = requests.post(
                    "https://resume-optimizer-api-12ok.onrender.com/feedback/submit",
                    json=payload
                )
                if res.status_code == 200:
